...
Request accepts:
  ...
  - currency
  - donor { name, email }  OR top-level name/email
...
Response (canonical camelCase):
  ...
  - publishableKey
  - mode
  - amountCents, feeCents, roundUpAddCents

Notes:
- CSRF-exempt (API-style JSON)
- Webhook returns 500 on processing failures (Stripe retries)
"""
...
def _create_stripe_event_row(*, event_id: str, etype: str, livemode: bool, object_id: str, payload_dict: Dict[str, Any]) -> StripeEvent:
    ...
    # If your model has a payload/json column, set it opportunistically without exploding.
    ...
    return _json_ok(
        {
            "platform": s.platform,
            "env": s.env,
            "stripeMode": s.stripe_mode,
            "stripeSecretLoaded": bool(s.stripe_sk and s.stripe_sk.startswith("sk_")),
            "stripePublishableLoaded": bool(s.stripe_pk and s.stripe_pk.startswith("pk_")),
            "webhookSecretLoaded": bool(s.stripe_whsec),
        }
    )
            ...
            "events": [
                {
                    ...
                    "updated_at": _iso(getattr(e, "updated_at", None)),
                }
                for e in events
            ],
        }
...
def stripe_intent():
    ...
    # Create donation record first (audit/debug even if Stripe fails)
    try:
...
def stripe_webhook():
    ...
    # If Stripe isn't configured in local/dev, don't hard-fail and create retry storms.
    try:
        s.init_stripe()
    except Exception:
        return ("", 200)
    ...
    def _store_event():
        db.session.add(_create_stripe_event_row(event_id=event_id, etype=etype, livemode=livemode, object_id=obj_id, payload_dict=ev))
        _tx_commit()
    ...
    try:
        if isinstance(obj, dict) and etype.startswith("payment_intent."):
            ...
            if donation_id_raw.isdigit():
                updated = _update_donation(Donation.id == int(donation_id_raw), status=status, pi_id=pi_id, paid=(status == "succeeded"))
            elif pi_id:
                updated = _update_donation(Donation.provider_intent_id == pi_id, status=status, pi_id=pi_id, paid=(status == "succeeded"))

            # If we can’t map it, keep webhook successful; event is stored for inspection.
            if not updated:
                return ("", 200)
