...
def main() -> int:
    ...
    if ff_env == "production" or app_env == "production":
    ...
    if secret_key and secret_key.lower() not in {"changeme", "replace_me", "replace-with-secret"} and len(secret_key) >= 32:
        ok("env:SECRET_KEY", f"Looks present ({len(secret_key)} chars)")
    else:
        fail("env:SECRET_KEY", "Missing or suspiciously weak SECRET_KEY")
    ...
    if stripe_secret.startswith("sk_live_") and stripe_publishable.startswith("pk_live_") and stripe_mode == "live":
        ...
        warn("stripe:keys", "Stripe keys not configured")

    if stripe_webhook.startswith("whsec_"):
        ok("stripe:webhook_secret", mask_secret(stripe_webhook))
    elif stripe_webhook:
        warn("stripe:webhook_secret", f"Present but unexpected format: {mask_secret(stripe_webhook)}")
    else:
        warn("stripe:webhook_secret", "Missing webhook signing secret")
    ...
    try:
        with app.test_client() as client:
            ...
            js_match = re.search(r'<link[^>]+rel="preload"[^>]+as="script"[^>]+href="([^"]*ff-app\.js[^"]*)"', html)
            ...
            if css_match:
                ...
                if (ff_env == "production" or app_env == "production") and css_v == "dev":
            ...
            if js_match:
                ...
                if (ff_env == "production" or app_env == "production") and js_v == "dev":
                    fail("assets:ff-app.js:version", f"Production JS still uses v=dev ({js_url})")
                elif js_v:
                    ok("assets:ff-app.js:version", js_v)
            else:
                warn("assets:ff-app.js:url", "Could not find preload link for ff-app.js in rendered HTML")
...
def print_report() -> None:
    ...
    if FAILURES:
        ...
        print("Result: PASS WITH WARNINGS — probably shippable, but inspect the yellow bits.")
    else:
