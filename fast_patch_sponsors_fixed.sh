#!/usr/bin/env bash
# fast_patch_sponsors_fixed.sh — safe deterministic sponsor-section replacement
# Usage: bash fast_patch_sponsors_fixed.sh
set -euo pipefail
...
# perform replacement using python (reads the tmp file we just wrote)
python3 - <<PY
...
print("Patched sponsors block in:", src)
PY

echo "Backup: $BAK"
echo "Done."
echo "Preview diff: git --no-pager diff --no-color -- app/templates/index.html | sed -n '1,200p'"
